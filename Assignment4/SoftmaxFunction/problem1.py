...
def compute_L(a,y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    L = float(-np.log(a[y].item() + 1e-10)) if a[y] > 0 else float('inf')
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    ## (dL/dz)/(dz/dW) = dL/dW using shaping to ensure (c,p) dimensions and avoid broadcasting issues
    dL_dW = np.multiply(dL_dz.reshape(-1, 1), dz_dW)
    #########################################
    return dL_dW
...
def compute_dL_db(dL_dz,dz_db):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    ## (dL/dz) * (dz/db) = dL/db ----- dz_db is all ones
    dL_db = dL_dz * dz_db
...
def update_W(W, dL_dW, alpha=0.001):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    W -= alpha * dL_dW[:W.shape[0], :W.shape[1]]
    #########################################
    return W
...
def predict(Xtest, W, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        a = compute_a(z)
        P[i] = a
        Y[i] = np.argmax(a)  # Get the predicted class label
